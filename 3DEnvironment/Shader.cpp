...
void Shader::init(const std::string& vertFile , const std::string& fragFile)
{
	...
	{
		glAttachShader(shaderProgram, shaders[i]); //add all  shaders to the shader program
	}
	...
	glValidateProgram(shaderProgram); //check the  program is valid
	CheckShaderError(shaderProgram, GL_VALIDATE_STATUS, true, "Error: Shader program not valid");
...
void Shader::init(const std::string& vertFile, const std::string& fragFile, const std::string& geometryFile)
{
	shaderProgram = glCreateProgram(); // create shader program 
	shaders[0] = CreateShader(LoadShader(vertFile), GL_VERTEX_SHADER); // create vertex shader
	shaders[1] = CreateShader(LoadShader(geometryFile), GL_GEOMETRY_SHADER); // create Geometry shader
